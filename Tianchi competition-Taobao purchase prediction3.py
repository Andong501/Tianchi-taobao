##Tianchi competition: Taobao purchase prediction version 4 medium
...
df_Xy_train = df_Xy.loc[(df_Xy.date>np.datetime64('2014-11-23')) & (df_Xy.date<np.datetime64('2014-12-17'))];
...
##samplize train set 1:6
X_train_sp0 = X_train.as_matrix();
y_train_sp0 = y_train.values.ravel();
#under-sample
rus = RandomUnderSampler(ratio={0:14400, 1:2400}, random_state=np.random.randint(0, 100, 1)[0]);
X_train_sp1, y_train_sp1 = rus.fit_sample(X_train_sp0, y_train_sp0);

##convert dfs to libsvm format and prepare for spark ml
dump_svmlight_file(X_train_sp1, y_train_sp1, 'train_svm.txt', zero_based=False, multilabel=False); 
dump_svmlight_file(X_cv, y_cv.values.ravel(),'cv_svm.txt', zero_based=False, multilabel=False);
dump_svmlight_file(X_test, y_test.values.ravel(), 'test_svm.txt', zero_based=False, multilabel=False);
...
cv_precision = metrics.precision_score(cv_pred_pd.label.values, cv_pred_pd.prediction.values);
cv_recall = metrics.recall_score(cv_pred_pd.label.values, cv_pred_pd.prediction.values);
cv_f1 = metrics.f1_score(cv_pred_pd.label.values, cv_pred_pd.prediction.values);

#submit
y_test_pair = y_test.reset_index(level=[0, 1]).drop(['buy_nextday'], axis=1);
test_submit = pd.concat([y_test_pair, test_pred_pd], axis=1);
test_submit = test_submit.loc[test_submit.prediction==1];
